...
New procedures in `document.ts` router (or new `espd.ts` router):
...
- Add "Δημιουργία ΕΕΕΣ/ESPD" button alongside existing document generation buttons
- Opens ESPD wizard in full-screen dialog or sheet
...
2. Update their coverage status to `COVERED` in the fakelos report
3. Log activity: "ESPD generated and linked to fakelos"
