...
import { SourceSelector } from '@/components/discovery/source-selector';
import { TENDER_SOURCES, SOURCE_CATEGORIES, getDefaultEnabledSourceIds } from '@/data/tender-sources';
import {
  Search,
  Import,
  ExternalLink,
...
export function DiscoveryResults({ onImport }: DiscoveryResultsProps) {
  ...
  const [selectedSources, setSelectedSources] = useState<string[]>(getDefaultEnabledSourceIds());
  ...
  function handleImport(tender: DiscoveredTender) {
    setImportingId(tender.referenceNumber || tender.title);
    onImport(tender);
  }
  ...
  return (
    <div className="flex gap-6">
      ...
      <div className="flex-1 space-y-4 min-w-0">
        ...
        {!searchParams && !isLoading && (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-muted-foreground/20 bg-white/30 dark:bg-white/[0.02] backdrop-blur-sm py-16 px-6 text-center">
            ...
            <p className="text-sm font-medium text-foreground">
              Επιλέξτε πηγές και πατήστε Αναζήτηση
            </p>
            <p className="mt-1.5 text-xs text-muted-foreground max-w-sm">
              Επιλέξτε τις πηγές που θέλετε να αναζητήσετε από το μενού αριστερά, στη συνέχεια πατήστε "Αναζήτηση" για να βρείτε διαγωνισμούς.
            </p>
          </div>
        )}
        ...
        {!isLoading && !error && searchParams && filteredTenders.length === 0 && (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-muted-foreground/20 bg-white/30 dark:bg-white/[0.02] backdrop-blur-sm py-16 px-6 text-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-blue-500/10 mb-4">
              <Radar className="h-7 w-7 text-blue-500" />
            </div>
            ...
            <p className="mt-1.5 text-xs text-muted-foreground max-w-sm">
              Δοκιμάστε να ενεργοποιήσετε περισσότερες πηγές ή να αφαιρέσετε τα φίλτρα budget.
            </p>
          </div>
        )}
        ...
        {!isLoading && filteredTenders.length > 0 && (
          <p className="text-xs text-muted-foreground">
            {filteredTenders.length} διαγωνισμοί βρέθηκαν
          </p>
        )}
        ...
        {!isLoading && filteredTenders.length > 0 && (
          <div className="space-y-3">
            {filteredTenders.map((tender, idx) => (
              <div
                key={`${tender.referenceNumber || idx}-${tender.sourceUrl}`}
                className={cn(
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    disabled={importingId === (tender.referenceNumber || tender.title)}
                    className={cn(
                  ...
                  >
                    {importingId === (tender.referenceNumber || tender.title) ? (
