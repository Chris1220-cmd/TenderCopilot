 ...
 * Country-aware: uses prompt context system for law references per country.
 *
...
function buildExtractClausesPrompt(ctx: CountryPromptContext): string {
  return `Είσαι νομικός σύμβουλος εξειδικευμένος στο δίκαιο δημοσίων συμβάσεων (${ctx.lawReference}).
...
function buildAssessRisksPrompt(ctx: CountryPromptContext): string {
  const euDirectivesStr = ctx.euDirectives.map(d => `Οδηγία ${d}`).join(', ');
  return `Είσαι ανώτερος νομικός σύμβουλος δημοσίων διαγωνισμών με βαθιά γνώση του ${ctx.lawReference}
και του ευρωπαϊκού δικαίου δημοσίων συμβάσεων (${euDirectivesStr}).
...
- Πληρωμή > 30 ημέρες (παραβίαση ${ctx.paymentTermReference})
...
function buildProposeClarificationsPrompt(ctx: CountryPromptContext): string {
  return `Είσαι νομικός σύμβουλος που συντάσσει ερωτήματα διευκρινίσεων σε αναθέτουσες αρχές
δημοσίων διαγωνισμών (${ctx.lawReference}).
...
4. Αναφέρεται σε νομοθεσία (${ctx.lawReference}, Οδηγίες ΕΕ) για τεκμηρίωση
...
class AILegalAnalyzer {
   ...
   */
  async extractClauses(tenderId: string, language: 'el' | 'en' | 'nl' = 'el', country: string = 'GR') {
    ...
    if (shouldChunk(contextText)) {
      ...
      for (let i = 0; i < chunks.length; i++) {
        const aiResult = await ai().complete({
          messages: [
            { role: 'system', content: buildExtractClausesPrompt(promptCtx) + `\n\n${langInstruction}` },
            {
              role: 'user',
   ...
   */
  async assessRisks(tenderId: string, country: string = 'GR') {
    const promptCtx = getPromptContext(country);
    await requireDocuments(tenderId);
    ...
    const aiResult = await ai().complete({
      messages: [
        { role: 'system', content: buildAssessRisksPrompt(promptCtx) },
        {
          role: 'user',
   ...
   */
  async proposeClarifications(tenderId: string, country: string = 'GR') {
    const promptCtx = getPromptContext(country);
    await requireDocuments(tenderId);
    const clauses = await db.legalClause.findMany({
      where: {
    ...
    const aiResult = await ai().complete({
      messages: [
        { role: 'system', content: buildProposeClarificationsPrompt(promptCtx) },
        {
          role: 'user',
   ...
   */
  async getLegalRiskSummary(tenderId: string, country: string = 'GR'): Promise<LegalRiskSummary> {
    const promptCtx = getPromptContext(country);
    const clauses = await db.legalClause.findMany({
      where: { tenderId },
      orderBy: [
    ...
    const FIELD_KEYWORDS: Record<string, string[]> = promptCtx.legalFieldKeywords;
