...
import { getPromptContext, type CountryPromptContext } from '@/lib/prompts';
...
function buildSummarizeSystemPromptEL(ctx: CountryPromptContext): string {
  return `Είσαι ειδικός σύμβουλος δημοσίων συμβάσεων. Αναλύεις κείμενα διαγωνισμών σύμφωνα με τον ${ctx.lawReference} και τον Ν.3130/2003 (στεγάσεις).
...

{
  ...
  "keyPoints": {
    ...
    "deadlines": [{"label": "Προθεσμία υποβολής", "date": "ISO date αν υπάρχει συγκεκριμένη ημερομηνία, ΑΛΛΙΩΣ γράψε ακριβώς τι αναφέρει το κείμενο (π.χ. 'εντός 5 εργασίμων ημερών από ανάρτηση στο ${ctx.platforms.join(', ')}')"}],
...
function getSummarizePrompt(documentText: string, ctx: CountryPromptContext): string {
  const lang = detectDocumentLanguage(documentText);
  console.log(`[BidOrchestrator] Document language detected: ${lang}`);
  return lang === 'el' ? buildSummarizeSystemPromptEL(ctx) : SUMMARIZE_SYSTEM_PROMPT_INTL;
}

function buildGoNoGoSystemPrompt(ctx: CountryPromptContext): string {
  return `Είσαι στρατηγικός σύμβουλος δημοσίων διαγωνισμών (Bid Director) με 20+ χρόνια εμπειρία
(${ctx.lawReference}, ${ctx.eProcurementPlatform}). Αξιολογείς αν μια εταιρεία πρέπει να συμμετάσχει σε διαγωνισμό.
...
function buildTeamRequirementsSystemPrompt(ctx: CountryPromptContext): string {
  return `Είσαι ειδικός σε ανθρώπινο δυναμικό δημοσίων διαγωνισμών (${ctx.lawReference}).
...
Πρόσεχε ιδιαίτερα:
- Κριτήρια ποιοτικής επιλογής (Άρθρο 75 ${ctx.lawReference}) — τεχνική/επαγγελματική ικανότητα
...
function buildSubcontractorNeedsSystemPrompt(ctx: CountryPromptContext): string {
  return `Είσαι ειδικός σύμβουλος δημοσίων συμβάσεων (${ctx.lawReference}).
...
function buildStatusSystemPrompt(_ctx: CountryPromptContext): string {
  return `Είσαι ο AI Bid Manager του TenderCopilot. Απαντάς σε ερωτήσεις κατάστασης
...
class AIBidOrchestrator {
   ...
   */
  async summarizeTender(tenderId: string, language: 'el' | 'en' | 'nl' = 'el', country: string = 'GR') {
    const _t0 = Date.now();
    ...
    try {
      ...
      const systemPrompt = getSummarizePrompt(fullText, promptCtx);
   ...
   */
  async goNoGoAnalysis(tenderId: string, tenantId: string, language: 'el' | 'en' | 'nl' = 'el', country: string = 'GR') {
    await requireDocuments(tenderId);
    ...
    const aiResult = await ai().complete({
      messages: [
        { role: 'system', content: buildGoNoGoSystemPrompt(getPromptContext(country)) + `\n\n${langInstruction}` },
        {
          role: 'user',
   ...
   */
  async analyzeTeamRequirements(tenderId: string, country: string = 'GR') {
    const tender = await db.tender.findUniqueOrThrow({
      where: { id: tenderId },
      include: {
        requirements: true,
    ...
    const aiResult = await ai().complete({
      messages: [
        { role: 'system', content: buildTeamRequirementsSystemPrompt(getPromptContext(country)) },
        {
          role: 'user',
   ...
   */
  async analyzeSubcontractorNeeds(tenderId: string, language: string = 'el', country: string = 'GR') {
    const tender = await db.tender.findUniqueOrThrow({
      where: { id: tenderId },
      include: {
        requirements: true,
    ...
    try {
      const aiResult = await ai().complete({
        messages: [
          { role: 'system', content: buildSubcontractorNeedsSystemPrompt(getPromptContext(country)) },
          {
            role: 'user',
   ...
   */
  async answerStatusQuestion(tenderId: string, question: string, country: string = 'GR'): Promise<StatusAnswer> {
    // Load all relevant data in parallel
    const [
      tender,
      tasks,
    ...
    const aiResult = await ai().complete({
      messages: [
        { role: 'system', content: buildStatusSystemPrompt(getPromptContext(country)) },
        {
          role: 'user',
